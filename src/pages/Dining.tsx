...
import {
  Star, MapPin, Search, X, SlidersHorizontal, Compass, Utensils,
  ArrowRight, Sparkles, Coffee, Beer, Building2, Globe,
...
const categories = [
  ...
  { label: 'Local', icon: Coffee, color: 'text-emerald-400' },
  { label: 'Continental', icon: Globe, color: 'text-sky-400' },
  { label: "Pub & Grill", icon: Beer, color: 'text-amber-400' },
  { label: 'Cafe', icon: Coffee, color: 'text-rose-400' },
  { label: 'Resort Dining', icon: Building2, color: 'text-purple-400' },
] as const
...
function getCategoryIcon(category: string) {
  switch(category) {
    case 'Local': return <Coffee className="w-3.5 h-3.5" />
    case 'Continental': return <Globe className="w-3.5 h-3.5" />
    case 'Pub & Grill': return <Beer className="w-3.5 h-3.5" />
    case 'Cafe': return <Coffee className="w-3.5 h-3.5" />
    case 'Resort Dining': return <Building2 className="w-3.5 h-3.5" />
...
function getCategoryColor(category: string) {
  switch(category) {
    case 'Local': return 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20'
    case 'Continental': return 'text-sky-400 bg-sky-500/10 border-sky-500/20'
    case 'Pub & Grill': return 'text-amber-400 bg-amber-500/10 border-amber-500/20'
    case 'Cafe': return 'text-rose-400 bg-rose-500/10 border-rose-500/20'
    case 'Resort Dining': return 'text-purple-400 bg-purple-500/10 border-purple-500/20'
...
export default function Dining() {
  ...
  const featured = [...DINING].sort((a, b) => (b.rating || 0) - (a.rating || 0))[0]

  const reviews = [
    { user: 'Kwame A.', text: 'The grilled tilapia at Eat & Feel is the best I\'ve had along the Volta. Fresh, perfectly spiced, and served with a view.', rating: 5, location: 'Accra, Ghana' },
    { user: 'Sarah M.', text: 'Soul Berry Cafe is a hidden gem. The coffee is excellent and the garden setting is perfect for a relaxing afternoon.', rating: 5, location: 'London, UK' },
    { user: 'Elena R.', text: 'Dining at The Royal Senchi was a world-class experience. The river terrace at sunset is unforgettable.', rating: 5, location: 'Madrid, Spain' },
    { user: 'David C.', text: 'The local chop bars serve the most authentic Ghanaian food. Don\'t miss the fufu and light soup at Jamrok.', rating: 4, location: 'Singapore' },
  ]
  ...
  return (
    <div className="min-h-screen bg-bg">
      <MirrorHero
        ...
        badge="Taste Asuogyaman"
        title="Where to Eat"
        description="From riverside fine dining at The Royal Senchi to authentic local chop bars serving fufu and waakye — experience every flavour Asuogyaman has to offer."
      ...
      <AnimatedSection className="py-16 px-5 relative border-b border-border/40">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-3 gap-8 text-center relative">
          <div>
            <div className="text-3xl md:text-4xl font-medium text-fg mb-1 text-gradient"><AnimatedCounter value={DINING.length} suffix="+" /></div>
            <div className="text-xs text-muted tracking-wide">Dining Options</div>
          </div>
          <div>
        ...
        <div className="max-w-7xl mx-auto px-5 py-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search dining..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted hover:text-fg transition-colors">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
                  ...
                  <motion.div initial={{ opacity: 0, y: -4, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.96 }} transition={{ duration: 0.15 }}
                    ...
                    {sortOptions.map((opt) => (
                      <button key={opt.value} onClick={() => { setSortBy(opt.value); setShowSort(false) }}
                        className={`w-full text-left px-4 py-2.5 text-xs transition-colors ${sortBy === opt.value ? 'text-accent bg-accent/5 font-medium' : 'text-muted hover:text-fg hover:bg-bg'}`}>
                        {opt.label}
                      </button>
                    ))}
                  </motion.div>
            ...
            {categories.map((cat) => {
              ...
              return (
                <motion.button key={cat.label} layout onClick={() => setActiveCategory(cat.label)}
                  ...
                  <cat.icon className={`w-3.5 h-3.5 ${isActive ? '' : cat.color}`} />
                  {cat.label}
                </motion.button>
              )
            })}
          ...
          <div className="max-w-7xl mx-auto">
            <RevealSection>
              <div className="flex items-end justify-between mb-6">
                <div>
                  <span className="text-[9px] font-semibold uppercase tracking-[0.3em] text-accent mb-2 block">Featured</span>
                  <h2 className="text-xl md:text-2xl font-medium text-fg tracking-tight">Top-Rated Dining</h2>
                </div>
              </div>
            </RevealSection>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8">
                  ...
                  <div className="flex items-center gap-3 mt-3 md:mt-4">
                    <div className="flex items-center gap-1 bg-accent/15 backdrop-blur-xl px-2.5 py-1 rounded-full">
                      <Star className="w-3 h-3 fill-accent text-accent" /><span className="text-[10px] font-semibold text-accent">{featured.rating}</span>
                    </div>
      ...
      <section className="py-8 md:py-12 px-5 pb-24 relative">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <p className="text-xs text-muted">{filtered.length} {filtered.length === 1 ? 'option' : 'options'} found</p>
            {activeCategory !== 'All' && (
              <button onClick={() => setActiveCategory('All')} className="text-xs text-accent hover:text-accent/80 transition-colors flex items-center gap-1">
          ...
          <AnimatePresence mode="wait">
            {filtered.length === 0 ? (
              <motion.div key="empty" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="text-center py-20">
                ...
                <p className="text-sm text-muted">No dining options match your search.</p>
      ...
      {activeCategory === 'All' && !searchQuery && (
        <>
          {categories.slice(1).map((cat) => {
            const items = DINING.filter((d) => d.category === cat.label)
              ...
              <AnimatedSection key={cat.label} className="py-16 md:py-20 px-5 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative">
                  <div className="absolute inset-0 rounded-3xl overflow-hidden opacity-[0.04] pointer-events-none">
                    <img src={cat.label === 'Local' ? '/Images/category-nature.jpg' : cat.label === 'Continental' ? '/Images/category-engineering.jpg' : '/Images/category-culture.jpg'} alt="" className="w-full h-full object-cover" loading="lazy" />
                  </div>
                  <RevealSection>
                    <div className="flex items-end justify-between mb-8">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-muted">{items.length} {items.length === 1 ? 'option' : 'options'}</p>
                        </div>
                      </div>
                      <button onClick={() => setActiveCategory(cat.label)} className="text-[10px] font-medium text-accent hover:text-accent/80 transition-colors flex items-center gap-1">
                              ...
                              <div className="flex items-start justify-between gap-2">
                                ...
                                {item.rating && (
                                  <div className="flex items-center gap-1 shrink-0 bg-accent/5 px-2 py-0.5 rounded-full">
                                    <Star className="w-3 h-3 fill-accent text-accent" /><span className="text-[10px] font-medium text-accent">{item.rating}</span>
                                  </div>
                                )}
                              </div>
      ...
      <AnimatedSection className="py-20 md:py-24 px-5 relative overflow-hidden bg-surface/50">
        <FloatingOrbs />
        <SectionDivider label="Reviews" className="mb-8" />
        <div className="max-w-3xl mx-auto text-center relative">
          <RevealSection>
            <h2 className="text-xl md:text-2xl font-medium text-fg tracking-tight mb-2">What Diners Say</h2>
            <p className="text-xs text-muted mb-10 max-w-md mx-auto">Real reviews from visitors who have dined across Asuogyaman.</p>
          </RevealSection>
              ...
              <motion.div key={reviewIndex} initial={{ opacity: 0, y: 10, filter: 'blur(4px)' }} animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, y: -10, filter: 'blur(4px)' }} transition={{ duration: 0.4, ease: easeOut }} className="space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-center gap-1 mb-2">
                    {Array.from({ length: reviews[reviewIndex].rating }).map((_, i) => (<Star key={i} className="w-3.5 h-3.5 fill-accent text-accent" />))}
                  </div>
          ...
          <div className="flex items-center justify-center gap-2 mt-8">
            {reviews.map((_, i) => (
              <button key={i} onClick={() => setReviewIndex(i)} className={`h-1.5 rounded-full transition-all duration-500 ${i === reviewIndex ? 'w-6 bg-accent' : 'w-1.5 bg-border hover:bg-muted'}`} />
            ))}
          </div>
        </div>
      </AnimatedSection>
      ...
      <AnimatePresence>
        {selectedItem && (
          <DiningDetailPanel item={selectedItem} onClose={() => setSelectedItem(null)} onNavigate={(id) => { const next = DINING.find((d) => d.id === id); if (next) setSelectedItem(next) }} />
        )}
      </AnimatePresence>
    </div>
  )
...
function DiningCard({ item, index, onClick }: { item: Dining; index: number; onClick: () => void }) {
          ...
          <div className="relative overflow-hidden" style={{ paddingBottom: '66%' }}>
            ...
            {item.rating && (
              <div className="absolute top-3 right-3 md:top-4 md:right-4 flex items-center gap-1 bg-black/40 backdrop-blur-xl px-2 py-0.5 rounded-full">
                <Star className="w-2.5 h-2.5 fill-accent text-accent" /><span className="text-[9px] font-semibold text-accent">{item.rating}</span>
              </div>
            )}
...
function DiningDetailPanel({ item, onClose, onNavigate }: { item: Dining; onClose: () => void; onNavigate: (id: string) => void }) {
  ...
  const currentIndex = DINING.findIndex((d) => d.id === item.id)
  const prevId = currentIndex > 0 ? DINING[currentIndex - 1].id : null
  const nextId = currentIndex < DINING.length - 1 ? DINING[currentIndex + 1].id : null

  useEffect(() => { if (scrollRef.current) scrollRef.current.scrollTop = 0 }, [item.id])

  const highlights = item.amenities?.slice(0, 5) || ['Great food', 'Excellent service', 'Unique atmosphere', 'Local ingredients', 'Family-friendly']
  const visitingTips = [
    { icon: Sun, label: 'Best Time', value: 'Lunch or dinner — check opening hours before visiting' },
    { icon: Clock, label: 'Cuisine', value: `${item.category} — ${item.description.split('.')[0]}` },
    { icon: Phone, label: 'Contact', value: item.phone || 'Contact for reservations' },
            ...
            <div className="max-w-7xl mx-auto px-5 md:px-8 lg:px-16 pb-8 md:pb-12 lg:pb-20">
              <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2, ease: easeOut }}>
                <div className="flex flex-wrap items-center gap-4 mb-5">
                  <span className="text-brand-gold text-[10px] uppercase tracking-[0.35em] font-bold">Dining Experience</span>
                  {item.rating && (
                    <div className="flex items-center gap-1.5 bg-brand-gold/20 backdrop-blur-xl border border-brand-gold/30 px-3 py-1 rounded-full">
                      <Star className="w-3 h-3 fill-brand-gold text-brand-gold" /><span className="text-[10px] font-black text-brand-gold">{item.rating}</span>
                    </div>
                  )}
                </div>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6 pb-12" style={{ minHeight: 'calc(100dvh - 70vh + 4rem)' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: easeOut }}
              ...
              <div className="flex items-start gap-6 mb-8">
                ...
                <div>
                  ...
                  <div className="text-white/30 text-[10px] uppercase tracking-[0.3em] font-bold">Discover the details</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-10">
                <div className="text-center p-4 rounded-xl bg-white/[0.04] border border-white/[0.06]">
                  ...
                  <div className="text-sm font-medium text-accent">{item.category}</div>
                </div>
                <div className="text-center p-4 rounded-xl bg-white/[0.04] border border-white/[0.06]">
                  <div className="text-white/40 text-[9px] uppercase tracking-[0.25em] font-bold mb-2">Rating</div>
                  <div className="text-sm font-medium text-brand-gold flex items-center justify-center gap-1"><Star className="w-4 h-4 fill-brand-gold" />{item.rating || 'N/A'}</div>
                </div>
                <div className="text-center p-4 rounded-xl bg-white/[0.04] border border-white/[0.06]">
                ...
                <div className="text-center p-4 rounded-xl bg-white/[0.04] border border-white/[0.06]">
                  <div className="text-white/40 text-[9px] uppercase tracking-[0.25em] font-bold mb-2">Contact</div>
                  <div className="text-sm font-medium text-white/80 truncate">{item.phone || 'N/A'}</div>
                </div>
              </div>
            </motion.div>
            ...
            {highlights.length > 0 && (
              <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} className="mb-8">
                <div className="flex items-center gap-5 mb-6">
                  ...
                  <div><h2 className="text-2xl font-serif text-white tracking-tight">Amenities & Features</h2><p className="text-white/30 text-[10px] uppercase tracking-[0.3em] font-bold mt-1">What to expect</p></div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.15 }} className="mb-8">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {item.phone && (
                  <a href={`tel:${item.phone}`} className="flex items-start gap-4 p-5 rounded-xl bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.08] transition-all group">
                ...
                {item.email && (
                  <a href={`mailto:${item.email}`} className="flex items-start gap-4 p-5 rounded-xl bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.08] transition-all group">
                ...
                {item.bookingUrl && (
                  <a href={item.bookingUrl} target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 p-5 rounded-xl bg-brand-gold/[0.08] border border-brand-gold/20 hover:bg-brand-gold/[0.12] transition-all group">
                    ...
                    <div className="min-w-0"><div className="text-[9px] uppercase tracking-[0.25em] font-bold text-brand-gold/60 mb-1">Book Online</div><div className="text-sm text-brand-gold">Make a Reservation</div></div>
                  </a>
                )}
                {item.coordinates && (
                  <a href={`https://www.google.com/maps/dir/?api=1&destination=${item.coordinates[0]},${item.coordinates[1]}`} target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 p-5 rounded-xl bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.08] transition-all group">
