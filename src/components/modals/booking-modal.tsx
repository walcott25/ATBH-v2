          ...
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative bg-white/80 backdrop-blur-2xl w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl border border-white/30">
            <div className="p-8 md:p-12 text-center">
              ...
              <div className="space-y-4">
                <a href="tel:+233240000000" className="block w-full bg-brand-dark text-white py-4 rounded-xl font-medium hover:bg-brand-gold transition-all">Call Reservations</a>
                <button onClick={() => { setIsBookingOpen(false); setIsChatOpen(true); }} className="block w-full border border-brand-dark/10 py-4 rounded-xl font-medium hover:bg-brand-cream transition-all">Chat with Concierge</button>
              </div>
            </div>
