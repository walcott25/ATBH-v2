...
export default function MirrorHero({ image, badge, title, description, cta }: MirrorHeroProps) {
  return (
    <section className="min-h-[50vh] md:min-h-screen flex flex-col md:flex-row">
      {/* Left — Image Panel */}
      <div
        className="relative w-full md:w-1/2 h-[40vh] md:h-screen flex flex-col justify-between p-8 md:p-14 overflow-hidden"
        style={{
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-t from-fg/95 via-fg/60 to-fg/30 pointer-events-none" />
        ...
        <div className="relative z-10">
          <motion.h1
            ...
            className="font-['Playfair_Display_SC'],serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-semibold text-white leading-[1.05] mb-3 tracking-wide"
          >
            {title}
          </motion.h1>
          <div className="w-16 h-0.5 bg-accent/60 mb-6" />
          {cta && (
            <motion.div
